...
public class AutoMidToMid {
	...
	public AutoMidToMid() {
		...
		while (Robot.driveSubsystem.getLeftEncoder().getDistance() < -87.63)
			Robot.driveSubsystem.drive(-0.3, -0.3);// backup halfway -27.94 cm

		// Note: errors may accumulate quickly - best to zero out encoders as
		...
		Robot.driveSubsystem.turnToHeading(90);// turn left

		Robot.driveSubsystem.getLeftEncoder().reset();
		Robot.driveSubsystem.getRightEncoder().reset();

		while (Robot.driveSubsystem.getLeftEncoder().getDistance() < 85)
			Robot.driveSubsystem.drive(0.5, 0.5);// drive forward parallel to
													// baseline
		...
		while (Robot.driveSubsystem.getLeftEncoder().getDistance() < 134)
			Robot.driveSubsystem.drive(0.5, 0.5);// drive forward past the
													// baseline
		...
		Robot.shootSubsystem.setShoot1(0.5);// will most likely have some
											// constant value
		// double shoot; -> need to come up with a way to adjust speed based on
		// driver input sensor
